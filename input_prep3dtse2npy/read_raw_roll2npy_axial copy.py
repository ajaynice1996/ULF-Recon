...
if __name__ == "__main__":
    ...
    nifti_img = nib.load('./DataSRR_AJ/SRR/1_ajayscan_leo/10mm/lf_mri_3DTSE_1.nii.gz')
    ...
    # --------------------------------
    vol_roll_z = np.roll(im, -9, axis=2)  # shift slices (Z-axis)
    ...
    vol_roll_y = np.roll(vol_roll_x, 4, axis=0)  # shift vertically (Y-axis)
    ...
    im_axial = np.moveaxis(im_resampled, [0, 1, 2], [1, 0, 2])  # yzx --> xyz
    ...
    make_nifti(
        im_resampled,
        fname='./DataSRR_AJ/SRR/1_ajayscan_leo/10mm/lf_mri_3DTSE_1_resampled.nii.gz',
        mask=False,
        res=target_spacing,
        dim_info=[0, 1, 2]
    )
    ...
    make_nifti(
        im_axial,
        fname='./DataSRR_AJ/SRR/1_ajayscan_leo/10mm/lf_mri_3DTSE_1_resampled_axial.nii.gz',
        mask=False,
        res=[2,2,2],
        dim_info=[0, 1, 2]
    )

    save_path_npy = './DataSRR_AJ/SRR/1_ajayscan_leo/npy1/axial_circshift_1zy.npy'
