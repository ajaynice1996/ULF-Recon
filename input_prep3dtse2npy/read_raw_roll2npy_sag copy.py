...
if __name__ == "__main__":
    ...
    nifti_img = nib.load('./DataSRR_AJ/SRR/1_ajayscan_leo/10mm/lf_mri_3DTSE_3.nii.gz')
    ...
    # -------------------------------
    # vol_roll_z = np.roll(im, -8, axis=2)  # shift slices (Z-axis)
    # visualize_volume(vol_roll_z, title="Roll Z (slices)")
    vol_roll_x = np.roll(im, 37, axis=1)   # shift horizontally (X-axis)
    visualize_volume(vol_roll_x, title="Roll X (horizontal)")
    vol_roll_y = np.roll(vol_roll_x, 7, axis=0)  # shift vertically (Y-axis)
    ...
    # im_resampled = np.rot90(im_resampled, k=-1, axes=(0, 1))  # Rotate in the plane of first and last axes
    ...
    make_nifti(
        im_resampled,
        fname='./DataSRR_AJ/SRR/1_ajayscan_leo/10mm/lf_mri_3DTSE_3_resampled.nii.gz',
        mask=False,
        res=target_spacing,
        dim_info=[0, 1, 2]
    )
    ...
    make_nifti(
        im_sag,
        fname='./DataSRR_AJ/SRR/1_ajayscan_leo/10mm/lf_mri_3DTSE_3_resampled_sag.nii.gz',
        mask=False,
        res=[2,2,2],
        dim_info=[0, 1, 2]
    )

    save_path_npy = './DataSRR_AJ/SRR/1_ajayscan_leo/npy1/sagittal_circshift_1yx.npy'
