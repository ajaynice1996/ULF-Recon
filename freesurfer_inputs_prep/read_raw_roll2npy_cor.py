...
if __name__ == "__main__":
    ...
    im = read_lf_data(
    data_folder='Training_DATA/Ajay_training/Ajay_training_06',
    output_folder='./DataSRR_AJ/SRR/ajay_training_06/arxiv_npy',
    subject="10mm",
    sub_folder='3DTSE/10',
    ...
    current_spacing = [2.0, 2.037037037037037, 10.0]  # Example current voxel size
    target_spacing = [2.0, 2.0, 5.0]

    # read saved nifti file to get im variable
    nifti_img = nib.load('./DataSRR_AJ/SRR/ajay_training_06/10mm/lf_mri_3DTSE_2.nii.gz')
    ...
    vol_roll_x = np.roll(vol_roll_z, 46, axis=1)   # shift horizontally (X-axis)
    visualize_volume(vol_roll_x, title="Roll X (horizontal)")
    vol_roll_y = np.roll(vol_roll_x, 2, axis=0)  # shift vertically (Y-axis)
    ...
    # im_resampled = np.rot90(im_resampled, k=-1, axes=(0, 1))
    ...
    make_nifti(
        im_resampled,
        fname='./DataSRR_AJ/SRR/ajay_training_06/10mm/lf_mri_3DTSE_2_resampled.nii.gz',
        mask=False,
        res=target_spacing,
        dim_info=[0, 1, 2]
    )
    ...
    make_nifti(
        im_cor,
        fname='./DataSRR_AJ/SRR/ajay_training_06/10mm/lf_mri_3DTSE_2_resampled_cor.nii.gz',
        mask=False,
        res=[2,2,2],
        dim_info=[0, 1, 2]
    )

    save_path_npy = './DataSRR_AJ/SRR/ajay_training_06/npy1/coronal_circshift_1zx.npy'
