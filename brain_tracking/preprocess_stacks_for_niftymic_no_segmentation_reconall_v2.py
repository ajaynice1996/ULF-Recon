#!/usr/bin/env python3
"""
Recon-all-inspired preprocessing for low-field orthogonal stacks without segmentation.
...
1) Raw baseline
2) Edge-preserving MRI denoise (anisotropic diffusion)
3) Rician noise-floor correction
4) N4 bias field correction
5) Conservative in-plane Gibbs suppression
6) Slice intensity harmonization (reduce motion-like slice jumps)
7) Outside-noise attenuation using robust foreground mask
8) Background ghost suppression (slice-wise outside-mask cleanup)
9) Brain-focused robust normalization
10) Final outside cleanup
"""
...
def process_stack(name: str, input_path: str, out_dir: str) -> StackReport:
    ...
    data2 = rician_noise_floor_correction(data1, m0)
    metrics["step2_rician_floor_correct"] = compute_metrics(data2)
    save_step_triplet(data2, "step2_rician_floor_correct", qc_dir, name)
    ...
    data4 = suppress_gibbs_ringing_inplane(data3, strength=0.06)
    metrics["step4_gibbs_suppress"] = compute_metrics(data4)
    save_step_triplet(data4, "step4_gibbs_suppress", qc_dir, name)

    data5 = slice_intensity_harmonize(data4, target_percentile=90.0)
    metrics["step5_slice_harmonize"] = compute_metrics(data5)
    save_step_triplet(data5, "step5_slice_harmonize", qc_dir, name)

    fg = robust_foreground_mask(data5, low_q=20.0)
    data6 = attenuate_outside_noise(data5, fg, edge_soft_sigma=1.2)
    metrics["step6_outside_noise_atten"] = compute_metrics(data6, fg)
    save_step_triplet(data6, "step6_outside_noise_atten", qc_dir, name)

    data7 = suppress_background_ghosts(data6, fg)
    metrics["step7_bg_ghost_suppress"] = compute_metrics(data7, fg)
    save_step_triplet(data7, "step7_bg_ghost_suppress", qc_dir, name)

    data8 = mri_robust_normalize(data7, fg)
    metrics["step8_norm"] = compute_metrics(data8, fg)
    save_step_triplet(data8, "step8_norm", qc_dir, name)

    data9 = final_outside_cleanup(data8, fg, outside_scale=0.05)
    metrics["step9_final_outside_cleanup"] = compute_metrics(data9, fg)
    save_step_triplet(data9, "step9_final_outside_cleanup", qc_dir, name)
    ...
    for axis, axis_name in [(2, "axial"), (1, "coronal"), (0, "sagittal")]:
        save_overlay_montage(
            ...
            fg.astype(np.uint8),
            axis,
            f"{name} | step9_final + fg mask | {axis_name}",
            os.path.join(qc_dir, f"step9_final_fg_overlay_{axis_name}.png"),
        )

    preproc_img = os.path.join(out_dir, f"{name}_preproc_no_seg_reconall_v2.nii.gz")
    fg_mask = os.path.join(out_dir, f"{name}_fgmask_no_seg_reconall_v2.nii.gz")
    save_nifti_like(img, data9, preproc_img, dtype=np.float32)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Recon-all-inspired no-seg stack preprocessing")
    p.add_argument("--input-dir", type=str, default="DataSRR/volunteer_xxx/10mm/npy/native_niftymic_inputs_thr40")
    p.add_argument("--output-dir", type=str, default="DataSRR/volunteer_xxx/10mm/npy/native_niftymic_inputs_thr40/no_seg_preproc_reconall_v2")
    p.add_argument("--fixed-fig-dir", type=str, default="DataSRR/volunteer_xxx/figures/no_seg_preproc_reconall_v2")
    return p.parse_args()
...
def main() -> None:
    ...
    for name, path in stacks.items():
        ...
        dst_qc = os.path.join(args.fixed_fig_dir, name)
        _mkdir(dst_qc)
        _clear_pngs(dst_qc)
    ...
    summary = {
        ...
        "pipeline": [
            "step0_raw",
            "step1_mri_denoise",
            "step2_rician_floor_correct",
            "step3_n4_bias_correct",
            "step4_gibbs_suppress",
            "step5_slice_harmonize",
            "step6_outside_noise_atten",
            "step7_bg_ghost_suppress",
            "step8_norm",
            "step9_final_outside_cleanup",
        ],
        "stacks": [asdict(r) for r in reports],
    }

    summary_path = os.path.join(args.output_dir, "preprocess_no_seg_reconall_v2_summary.json")
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2)

    print("=" * 80)
    print("Finished reconall-style no-seg preprocessing (v2)")
    print("=" * 80)
