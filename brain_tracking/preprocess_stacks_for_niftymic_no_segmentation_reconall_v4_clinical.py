#!/usr/bin/env python3
"""
Light recon-all-inspired preprocessing for low-field orthogonal stacks without segmentation.
...
4) Slice intensity harmonization (reduce motion-like slice jumps)
5) Outside-noise attenuation using robust foreground mask (light)
6) Background ghost suppression (slice-wise outside-mask cleanup)
7) Brain-focused robust normalization
8) Final outside cleanup
...
def process_stack(name: str, input_path: str, out_dir: str) -> StackReport:
    ...
    data4 = attenuate_outside_noise(data3, fg, edge_soft_sigma=0.6)
    metrics["step4_outside_noise_atten_light"] = compute_metrics(data4, fg)
    save_step_triplet(data4, "step4_outside_noise_atten_light", qc_dir, name)

    data5 = suppress_background_ghosts(data4, fg)
    metrics["step5_bg_ghost_suppress"] = compute_metrics(data5, fg)
    save_step_triplet(data5, "step5_bg_ghost_suppress", qc_dir, name)

    data6 = mri_robust_normalize(data5, fg)
    metrics["step6_norm"] = compute_metrics(data6, fg)
    save_step_triplet(data6, "step6_norm", qc_dir, name)

    data7 = final_outside_cleanup(data6, fg, outside_scale=0.12)
    metrics["step7_final_outside_cleanup"] = compute_metrics(data7, fg)
    save_step_triplet(data7, "step7_final_outside_cleanup", qc_dir, name)
    ...
    for axis, axis_name in [(2, "axial"), (1, "coronal"), (0, "sagittal")]:
        save_overlay_montage(
            ...
            fg.astype(np.uint8),
            axis,
            f"{name} | step7_final + fg mask | {axis_name}",
            os.path.join(qc_dir, f"step7_final_fg_overlay_{axis_name}.png"),
        )

    preproc_img = os.path.join(out_dir, f"{name}_preproc_no_seg_reconall_v4_clinical_base.nii.gz")
    fg_mask = os.path.join(out_dir, f"{name}_fgmask_no_seg_reconall_v4_clinical.nii.gz")
    save_nifti_like(img, data7, preproc_img, dtype=np.float32)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Clinical-style recon-all-inspired no-seg stack preprocessing")
    p.add_argument("--input-dir", type=str, default="DataSRR/volunteer_xxx/10mm/npy/native_niftymic_inputs_thr40")
    p.add_argument("--output-dir", type=str, default="DataSRR/volunteer_xxx/10mm/npy/native_niftymic_inputs_thr40/no_seg_preproc_reconall_v4_clinical")
    p.add_argument("--fixed-fig-dir", type=str, default="DataSRR/volunteer_xxx/figures/no_seg_preproc_reconall_v4_clinical")
    return p.parse_args()
...
def main() -> None:
    ...
    for name, rep in rep_map.items():
        ...
        rep.metrics_by_step["step8_cross_stack_harmonize"] = compute_metrics(harm, src_mask > 0)
        save_step_triplet(harm, "step8_cross_stack_harmonize", rep.outputs["qc_dir"], name)

        final_path = os.path.join(args.output_dir, name, f"{name}_preproc_no_seg_reconall_v4_clinical.nii.gz")
    ...
    summary = {
        ...
        "pipeline": [
            "step0_raw",
            "step1_clip_only",
            "step2_n4_bias_correct",
            "step3_slice_harmonize",
            "step4_outside_noise_atten_light",
            "step5_bg_ghost_suppress",
            "step6_norm",
            "step7_final_outside_cleanup",
            "step8_cross_stack_harmonize",
        ],
        "stacks": [asdict(r) for r in reports],
    }

    summary_path = os.path.join(args.output_dir, "preprocess_no_seg_reconall_v4_clinical_summary.json")
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2)

    print("=" * 80)
    print("Finished clinical-style reconall-inspired no-seg preprocessing (v4)")
    print("=" * 80)
