...
Experimental cleanup step:
8b) Outside speckle suppression (stronger cleanup away from the head boundary)
"""
...
def outside_speckle_cleanup_experimental(
    data: np.ndarray,
    fg_mask: np.ndarray,
    floor_scale: float = 0.08,
    boundary_protect_dilate: int = 2,
    bright_quantile: float = 86.0,
    max_small_component_vox: int = 96,
) -> np.ndarray:
    """Suppress small bright outside islands while protecting head boundaries."""
    out = data.copy().astype(np.float32)
    fg = fg_mask > 0
    if not np.any(fg):
        return out
    ...
    floor = max(0.0, float(np.percentile(fg_vals, 2.0)) * float(floor_scale))

    protect = fg.copy()
    if binary_dilation is not None and boundary_protect_dilate > 0:
        for _ in range(boundary_protect_dilate):
            protect = binary_dilation(protect)

    outside = ~fg
    far_outside = outside & (~protect)
    sample = out[far_outside] if np.count_nonzero(far_outside) > 200 else out[outside]
    if sample.size == 0:
        return out

    qv = float(np.percentile(sample, np.clip(bright_quantile, 50.0, 99.5)))
    spike_thr = max(qv, floor * 1.25)
    spike_mask = far_outside & (out > spike_thr)

    if label is not None and np.any(spike_mask):
        cc, n_cc = label(spike_mask)
        for cc_id in range(1, n_cc + 1):
            comp = cc == cc_id
            nvox = int(np.count_nonzero(comp))
            if nvox <= max_small_component_vox:
                out[comp] = floor
            else:
                out[comp] = np.minimum(out[comp], spike_thr).astype(np.float32)
    else:
        out[spike_mask] = floor

    # Homogenize far-outside background to reduce residual hole-like texture.
    bg_median = float(np.median(sample))
    bg_target = float(np.clip(bg_median, 0.0, spike_thr))
    out[far_outside] = bg_target

    # Keep near-brain outside region conservative.
    near_outside = outside & protect
    out[near_outside] = np.minimum(out[near_outside], spike_thr)
    out[outside] = np.maximum(out[outside], 0.0)
    return out.astype(np.float32)
...
def process_stack(name: str, input_path: str, out_dir: str) -> StackReport:
    ...
    data8b = outside_speckle_cleanup_experimental(
        data8,
        fg,
        floor_scale=0.08,
        boundary_protect_dilate=2,
        bright_quantile=86.0,
        max_small_component_vox=96,
    )
    metrics["step8b_outside_speckle_cleanup_exp"] = compute_metrics(data8b, fg)
    save_step_triplet(data8b, "step8b_outside_speckle_cleanup_exp", qc_dir, name)
    ...
    for axis, axis_name in [(2, "axial"), (1, "coronal"), (0, "sagittal")]:
        save_overlay_montage(
            ...
            fg.astype(np.uint8),
            axis,
            f"{name} | step8b cleanup exp + fg mask | {axis_name}",
            os.path.join(qc_dir, f"step8b_cleanup_exp_fg_overlay_{axis_name}.png"),
        )
    ...
    save_nifti_like(img, data8b, preproc_img, dtype=np.float32)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output-dir", type=str, default="DataSRR/volunteer_xxx/10mm/npy/native_niftymic_inputs_thr40/reconall_v7_no_seg_outside_cleanup_exp")
    p.add_argument("--fixed-fig-dir", type=str, default="DataSRR/volunteer_xxx/figures/reconall_v7_no_seg_outside_cleanup_exp")
    return p.parse_args()
...
def main() -> None:
    ...
    for name, rep in rep_map.items():
        ...
        final_path = os.path.join(args.output_dir, name, f"{name}_preproc_reconall_v7_no_seg.nii.gz")
    ...
    summary = {
        ...
        "pipeline": [
            ...
            "step8b_outside_speckle_cleanup_exp",
            "step9_cross_stack_harmonize",
        ],
        "stacks": [asdict(r) for r in reports],
    }

    summary_path = os.path.join(args.output_dir, "preprocess_reconall_v7_no_seg_outside_cleanup_exp_summary.json")
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2)

    print("=" * 80)
    print("Finished clinical-style preprocessing with stronger outside cleanup experiment (no segmentation, v7)")
    print("=" * 80)
