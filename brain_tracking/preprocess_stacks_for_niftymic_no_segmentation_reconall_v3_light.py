...
def process_stack(name: str, input_path: str, out_dir: str) -> StackReport:
    ...
    preproc_img = os.path.join(out_dir, f"{name}_preproc_no_seg_reconall_v3_light.nii.gz")
    fg_mask = os.path.join(out_dir, f"{name}_fgmask_no_seg_reconall_v3_light.nii.gz")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Light recon-all-inspired no-seg stack preprocessing")
    p.add_argument("--input-dir", type=str, default="DataSRR/volunteer_xxx/10mm/npy/native_niftymic_inputs_thr40")
    p.add_argument("--output-dir", type=str, default="DataSRR/volunteer_xxx/10mm/npy/native_niftymic_inputs_thr40/no_seg_preproc_reconall_v3_light")
    p.add_argument("--fixed-fig-dir", type=str, default="DataSRR/volunteer_xxx/figures/no_seg_preproc_reconall_v3_light")
    return p.parse_args()
...
def main() -> None:
    ...
    summary_path = os.path.join(args.output_dir, "preprocess_no_seg_reconall_v3_light_summary.json")
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2)

    print("=" * 80)
    print("Finished light reconall-style no-seg preprocessing (v3)")
    print("=" * 80)
