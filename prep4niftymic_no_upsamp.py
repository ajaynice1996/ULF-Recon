...
write_shell_script(REFINED_SCRIPT_PATH, cmd)

# Run from Python
run_command_streaming(
    cmd,
    log_path=REFINED_LOG_PATH,
    cwd=os.getcwd(),
)

if not os.path.exists(REFINED_RECON_PATH):
    raise FileNotFoundError(f"Refined reconstruction not found: {REFINED_RECON_PATH}")

refined_img = nib.load(REFINED_RECON_PATH)

inspect_nifti(refined_img, label="refined SRR")

qc_png = os.path.join(
    QC_DIR,
    "refined_srr_central_slices.png",
)

save_recon_qc_png(
    refined_img,
    save_path=qc_png,
    title="Refined SRR: 2 cycles, outlier rejection, HuberL2, alpha 0.03",
...
# Optional comparison with geometry-debug reconstruction if variable/path exists.
geometry_debug_path = os.path.join(
    NIFTYMIC_INPUT_DIR,
    "srr_native_thr40_geometry_debug_2mm.nii.gz",
)

if os.path.exists(geometry_debug_path):
    geometry_img = nib.load(geometry_debug_path)
    compare_two_recons(
        geometry_img,
        refined_img,
        label_a="geometry-debug",
        label_b="refined",
    )
...
print(REFINED_RECON_PATH)
print("QC PNG:")
print(qc_png)
print("Log:")
print(REFINED_LOG_PATH)