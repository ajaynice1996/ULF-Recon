...
dataFolder = r'./Data/Data/In_vivo/2_avg/'
subjectID = 'sub_005'
outputfolder = os.path.join(r'./Data/Outputs', subjectID)
...
# Print shapes
print(im_axial.shape)
print(im_sag.shape)
print(im_cor.shape)

# im_axial = do_resize(im_data=im_axial, dim=[16, 110, 110])
# im_sag = do_resize(im_data=im_sag, dim=[110, 110, 16])
# im_cor = do_resize(im_data=im_cor, dim=[110, 16, 110])
...
axial_path = os.path.join(outputfolder, fname)
make_nifti(data=im_axial, fname=axial_path, mask=False,
...
sag_path = os.path.join(outputfolder, fname)
make_nifti(data=im_sag, fname=sag_path, mask=False,
...
cor_path = os.path.join(outputfolder, fname)
make_nifti(data=im_cor, fname=cor_path, mask=False,
...
axial_mask_path = os.path.join(outputfolder, fname)
make_nifti(data=im_axial_mask, fname=axial_mask_path, mask=True,
               res=[2, 2, 2], dim_info=[2, 1, 0])  # phase, freq, slice

# Visualize the image and mask to confirm correctness
plot_anatomy_raw(im_axial, clim=[0, 2048])
im_axial_mask = im_axial_mask.astype(np.float32)
# plot_anatomy_raw(im_axial_mask, clim=[0, 1])
...
sag_mask_path = os.path.join(outputfolder, fname)
make_nifti(data=im_sag_mask, fname=sag_mask_path, mask=True,
               res=[2, 2, 2], dim_info=[2, 1, 0])  # phase, freq, slice

# Visualize the image and mask to confirm correctness
plot_anatomy_raw(im_sag, clim=[0, 2048])
im_sag_mask = im_sag_mask.astype(np.float32)
# plot_anatomy_raw(im_sag_mask, clim=[0, 2048])
...
cor_mask_path = os.path.join(outputfolder, fname)
make_nifti(data=im_cor_mask, fname=cor_mask_path, mask=True,
               res=[2, 2, 2], dim_info=[2, 1, 0])  # phase, freq, slice

# Visualize the image and mask to confirm correctness
plot_anatomy_raw(im_cor, clim=[0, 2048])
im_cor_mask = im_sag_mask.astype(np.float32)
# plot_anatomy_raw(im_cor_mask, clim=[0, 2048])

niftymic_output = os.path.join(outputfolder, 'srr_1cycle_2mm_Huber_test2.nii.gz')
zssr_fname = os.path.join(outputfolder, 'srr_zssr_3D_full.nii.gz')

# NiftyMIC implementation
if niftymic:
    ...
    docker_command = [
        ...
        "--filenames-masks", 
        axial_mask_path, cor_mask_path, sag_mask_path,
        "--alpha", "0.02",
        "--outlier-rejection", "0",
        "--threshold-first", "0.5",
        "--threshold", "0.7",
...
# Output: SRR
if visualize:
    print('Loading NiftyMIC SRR output ..........')
