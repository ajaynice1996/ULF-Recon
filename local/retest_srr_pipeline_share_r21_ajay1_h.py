...
# REORIENT TO XYZ
# =========================
im_axial = np.moveaxis(im_axial, [0,1,2], [0,1,2])  # yzx → xyz
...
make_nifti(im_sag,   sag_path,   mask=False, res=[2,2,2], dim_info=[2,1,0])
...
make_nifti(sg_mask, sg_mask_path, mask=True, res=[2,2,2], dim_info=[2,1,0])
...
if visualize:
    plot_anatomy_raw(im_axial, clim=[0,2048])
    plot_anatomy_raw(ax_mask, clim=[0,1])
    plot_anatomy_raw(im_sag,   clim=[0,2048])
    plot_anatomy_raw(sg_mask,   clim=[0,1])
    plot_anatomy_raw(im_cor,   clim=[0,2048])
    plot_anatomy_raw(cr_mask,   clim=[0,1])
